...
public class BufferDataEventsProcessor implements Processor<String, DataEvent, String, List<DataEvent>> {

    private static final Logger log = LoggerFactory.getLogger(BufferDataEventsProcessor.class);
    ...
    private KeyValueStore<String, List<DataEvent>> kvStore;

    public BufferDataEventsProcessor(String fileStoreName, Integer maxBufferRecords, String maxBufferTime) {
    ...
    public void process(Record<String, DataEvent> record) {
        ...
        if (null != record.value() && StringUtils.isNotEmpty(event.type())) {
            var key  = event.namespace() + "|" +event.type();
            var existing = kvStore.get(key);
            var list = null != existing ? new ArrayList<DataEvent>(existing) : new ArrayList<DataEvent>(1);
            list.add(event);
            kvStore.put(key, list);
            log.info("Received data event {}", key);
            if (list.size() >= maxBufferRecords) {
                ...
                context.forward(new Record<>(key, copiedList, record.timestamp()));
            }
        }
    }

    @Override
    public void init(ProcessorContext<String, List<DataEvent>> context) {
        ...
        Punctuator punctuator = timestamp -> {
            ...
            try (var itr = kvStore.all()) {
                var records = new ArrayList<Record<String, List<DataEvent>>>(5);
                while(itr.hasNext()) {
                    var kv = itr.next();
