...
@KafkaListener(offsetReset = OffsetReset.EARLIEST,
        properties = { @Property(name = ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG,
                value = "com.dataplatform.processor.consumers.tranformers.DataEventListDeserializer"),
...
public class StoreDataEventsConsumer {

    private static final Logger log = LoggerFactory.getLogger(StoreDataEventsConsumer.class);
    ...
    public StoreDataEventsConsumer(IngestDataService ingestRawDataService,
    ...
    @Topic(Constants.Topic.STORE_DATA_EVENTS)
    public void receive(ConsumerRecord<String, List<DataEvent>> record) throws Exception {
        if (CollectionUtils.isNotEmpty(record.value())) {
            try {
                ...
                log.info("Received total {} grouped events of type [{}:{}]", events.size(), namespace, eventType);

                var records = toGenericRecords(namespace, eventType, events);
                var stagedFileData = ingestRawDataService.storeData(namespace, eventType, records);
                Optional.ofNullable(stagedFileData).ifPresent(staged ->
                    kafkaClientDelegate.send(namespace + "|" + eventType, staged)
                );
            } catch (Exception e) {
    ...
    private List<GenericRecord> toGenericRecords(String namespace, String tableName, List<DataEvent> events) throws ExecutionException {
